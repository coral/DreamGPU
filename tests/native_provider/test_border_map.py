...
struct gl_texture_image {uint8_t *border_data;unsigned Width,Height,TexFormat;struct pipe_transfer border_transfer;};
...
return NULL;}
int main(void){
 uint8_t pixels[24];memset(pixels,0x5a,sizeof pixels);struct gl_texture_image im={pixels,6,1,1,{0,0}};struct pipe_transfer *t=NULL;
 ...
 uint8_t plane[96];memset(plane,0x33,sizeof plane);im.border_data=plane;im.Height=4;im.border_transfer.usage=0;
 assert(map(&im,2,5,3,0,1,1,1,&t)==plane+92&&t->stride==24);im.border_transfer.usage=0;
 assert(!map(&im,2,5,3,0,1,2,1,&t)&&!im.border_transfer.usage);
 assert(!map(&im,2,0,(unsigned)-1,0,1,1,1,&t)&&!im.border_transfer.usage);
 assert(!map(&im,2,0,4,0,1,0,1,&t)&&!im.border_transfer.usage);
 assert(map(&im,1,1,1,0,4,2,1,&t)==plane+28&&t->stride==24);
 for(unsigned i=0;i<96;i++)assert(plane[i]==0x33);
