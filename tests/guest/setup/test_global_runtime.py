...
with tempfile.TemporaryDirectory(prefix="dreamgpu-global-runtime-") as directory:
    binary = Path(directory) / "global-runtime-tests"
    subprocess.run([compiler, "-std=c++23", "-Wall", "-Wextra", "-Werror",
                    "-fsanitize=address,undefined", "-fno-omit-frame-pointer",
                    str(ROOT / "tools/setup/global-runtime-tests.cpp"), "-o", str(binary)], check=True)
