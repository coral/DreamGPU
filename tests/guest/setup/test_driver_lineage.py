...
"""Run the production driver lineage policy under sanitizers."""
import os
...
with tempfile.TemporaryDirectory(prefix='dreamgpu-driver-lineage-') as directory:
    binary = Path(directory) / 'driver-tests'
    subprocess.run([compiler, '-std=c++23', '-Wall', '-Wextra', '-Werror',
                    '-fsanitize=address,undefined', '-fno-omit-frame-pointer',
                    str(ROOT / 'tools/setup/driver-lineage-tests.cpp'), '-o', str(binary)], check=True)
