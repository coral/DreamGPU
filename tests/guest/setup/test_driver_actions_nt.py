...
"""Run the production driver generation actions under sanitizers."""
import os
...
with tempfile.TemporaryDirectory(prefix='dreamgpu-driver-actions-nt-') as directory:
    binary = Path(directory) / 'driver-tests'
    subprocess.run([compiler, '-std=c++23', '-Wall', '-Wextra', '-Werror',
                    '-fsanitize=address,undefined', '-fno-omit-frame-pointer',
                    str(ROOT / 'tools/setup/driver-actions-nt-tests.cpp'), '-o', str(binary)], check=True)
