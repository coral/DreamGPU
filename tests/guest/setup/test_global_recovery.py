...
with tempfile.TemporaryDirectory(prefix="dreamgpu-global-recovery-") as directory:
    binary = Path(directory) / "global-recovery-tests"
    subprocess.run([compiler, "-std=c++23", "-Wall", "-Wextra", "-Werror",
                    "-fsanitize=address,undefined", "-fno-omit-frame-pointer",
                    str(ROOT / "tools/setup/global-recovery-tests.cpp"), "-o", str(binary)], check=True)
