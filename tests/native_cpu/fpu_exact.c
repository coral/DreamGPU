...
static void pair(floatx80 a, floatx80 b, float_status status) {
    for (unsigned quiet = 0; quiet < 2; ++quiet) {
        ...
        FloatRelation expected =
            quiet ? floatx80_compare_quiet(a, b, &ref) : floatx80_compare(a, b, &ref);
    ...
    for (FpuPc24Op op = FPU_PC24_ADD; op <= FPU_PC24_DIV; ++op) {
        ...
        switch (op) {
            case FPU_PC24_ADD:
                expected = floatx80_add(a, b, &ref);
                break;
            case FPU_PC24_SUB:
                expected = floatx80_sub(a, b, &ref);
                break;
            case FPU_PC24_MUL:
                expected = floatx80_mul(a, b, &ref);
                break;
            case FPU_PC24_DIV:
                expected = floatx80_div(a, b, &ref);
                break;
            default:
                abort();
        }
        ...
        if (expected.high != actual.high || expected.low != actual.low ||
            ...
            fprintf(stderr,
                    "op=%d pc=%d mode=%d a=%04x:%016" PRIx64 " b=%04x:%016" PRIx64
                    " expected=%04x:%016" PRIx64 "/%x actual=%04x:%016" PRIx64 "/%x\n",
                    op, get_floatx80_rounding_precision(&status), get_float_rounding_mode(&status),
                    a.high, a.low, b.high, b.low, expected.high, expected.low,
                    get_float_exception_flags(&ref), actual.high, actual.low,
                    get_float_exception_flags(&got));
            abort();
        }
        ++checks;
    }
