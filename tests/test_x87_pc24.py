...
"""Compare actual x87 PC24 helper against unchanged QEMU SoftFloat; needs native build."""
import argparse
import json
...
def main():
    ...
    command += ['-ffp-contract=off', str(ROOT / 'tests/native_cpu/fpu_pc24.c'),
                str(build / entry['output'])]
    command += shlex.split(subprocess.check_output(['pkg-config', '--libs', 'glib-2.0'], text=True))
    command += ['-lm', '-o', str(output / 'pc24-test')]
    (output / 'command.json').write_text(json.dumps(command, indent=2) + '\n')
    subprocess.run(command, cwd=build, check=True)
    subprocess.run([str(output / 'pc24-test')], check=True)
