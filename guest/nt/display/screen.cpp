/*
 * Source: vendor/reactos/win32ss/drivers/displays/framebuf/screen.c
...
static LOGFONTW SystemFont = {16,
                              ...
                              L"System"};
static LOGFONTW AnsiVariableFont = {12,
                                    9,
                                    0,
                                    0,
                                    400,
                                    ...
                                    VARIABLE_PITCH | FF_DONTCARE,
                                    L"MS Sans Serif"};
static LOGFONTW AnsiFixedFont = {12,
                                 9,
                                 0,
                                 0,
                                 400,
...
DWORD
GetAvailableModes(HANDLE hDriver, PVIDEO_MODE_INFORMATION *ModeInfo, DWORD *ModeInfoSize) {
    ULONG ulTemp;
    ...
    if (EngDeviceIoControl(hDriver, IOCTL_VIDEO_QUERY_NUM_AVAIL_MODES, NULL, 0, &Modes,
                           sizeof(VIDEO_NUM_MODES), &ulTemp)) {
        return 0;
    }
    ...
    *ModeInfo = (PVIDEO_MODE_INFORMATION)EngAllocMem(
    ...
    if (EngDeviceIoControl(hDriver, IOCTL_VIDEO_QUERY_AVAIL_MODES, NULL, 0, *ModeInfo,
                           Modes.NumModes * Modes.ModeInformationLength, &ulTemp)) {
        EngFreeMem(*ModeInfo);
        *ModeInfo = NULL;
        return 0;
    }
    ...
    while (ulTemp--) {
        if ((ModeInfoPtr->NumberOfPlanes != 1) ||
            ...
            ((ModeInfoPtr->BitsPerPlane != 8) && (ModeInfoPtr->BitsPerPlane != 16) &&
             (ModeInfoPtr->BitsPerPlane != 24) && (ModeInfoPtr->BitsPerPlane != 32))) {
            ModeInfoPtr->Length = 0;
        }

        ModeInfoPtr =
            (PVIDEO_MODE_INFORMATION)(((PUCHAR)ModeInfoPtr) + Modes.ModeInformationLength);
    }
...
BOOL IntInitScreenInfo(PPDEV ppdev, LPDEVMODEW pDevMode, PGDIINFO pGdiInfo, PDEVINFO pDevInfo) {
    ...
    if (pDevMode->dmPelsWidth == 0 && pDevMode->dmPelsHeight == 0 && pDevMode->dmBitsPerPel == 0 &&
        ...
        ModeInfoPtr = ModeInfo;
        while (ModeCount-- > 0) {
            if (ModeInfoPtr->Length > 0 && pDevMode->dmPelsWidth == ModeInfoPtr->VisScreenWidth &&
    ...
    if (!EngDeviceIoControl(ppdev->hDriver, IOCTL_VIDEO_QUERY_COLOR_CAPABILITIES, NULL, 0,
                            &ColorCapabilities, sizeof(VIDEO_COLOR_CAPABILITIES), &Temp)) {
