/*
 * Source: vendor/reactos/win32ss/drivers/displays/framebuf/enable.c
...
DHPDEV APIENTRY DrvEnablePDEV(IN DEVMODEW *pdm, IN LPWSTR pwszLogAddress, IN ULONG cPat,
    ...
    ppdev->WindowLock = EngCreateSemaphore();
    if (!ppdev->WindowLock) {
        EngFreeMem(ppdev);
        return NULL;
    }
    ...
    if (!IntInitScreenInfo(ppdev, pdm, &GdiInfo, &DevInfo)) {
        EngDeleteSemaphore(ppdev->WindowLock);
        EngFreeMem(ppdev);
        return NULL;
    }
    ...
    memcpy(pdi, &DevInfo, (cjDevInfo < sizeof(DEVINFO) ? cjDevInfo : sizeof(DEVINFO)));
    memcpy(pdevcaps, &GdiInfo, (cjCaps < sizeof(GDIINFO) ? cjCaps : sizeof(GDIINFO)));
...
VOID APIENTRY DrvDisablePDEV(IN DHPDEV dhpdev) {
    DgDeleteWindows((PPDEV)dhpdev);
    EngDeleteSemaphore(((PPDEV)dhpdev)->WindowLock);
