/*
 * Source: vendor/reactos/win32ss/drivers/displays/framebuf/palette.c
...
const PALETTEENTRY BASEPALETTE[20] = {
    ...
    {0x00, 0x80, 0x80, 0x00}, {0xC0, 0xC0, 0xC0, 0x00}, {0xC0, 0xDC, 0xC0, 0x00},
    {0xD4, 0xD0, 0xC8, 0x00}, {0xFF, 0xFB, 0xF0, 0x00}, {0x3A, 0x6E, 0xA5, 0x00},
    {0x80, 0x80, 0x80, 0x00}, {0xFF, 0x00, 0x00, 0x00}, {0x00, 0xFF, 0x00, 0x00},
...
BOOL IntInitDefaultPalette(PPDEV ppdev, PDEVINFO pDevInfo) {
    ...
    if (ppdev->BitsPerPixel > 8) {
        ppdev->DefaultPalette = pDevInfo->hpalDefault = EngCreatePalette(
            PAL_BITFIELDS, 0, NULL, ppdev->RedMask, ppdev->GreenMask, ppdev->BlueMask);
    } else {
        ppdev->PaletteEntries =
            (PALETTEENTRY *)EngAllocMem(0, sizeof(PALETTEENTRY) << 8, ALLOC_TAG);
        if (ppdev->PaletteEntries == NULL) {
            return FALSE;
        }

        for (ColorLoop = 256, PaletteEntryPtr = ppdev->PaletteEntries; ColorLoop != 0;
...
BOOL APIENTRY IntSetPalette(IN DHPDEV dhpdev, IN PPALETTEENTRY ppalent, IN ULONG iStart,
    ...
    pClut = (PVIDEO_CLUT)EngAllocMem(0, ClutSize, ALLOC_TAG);
    ...
    if (EngDeviceIoControl(((PPDEV)dhpdev)->hDriver, IOCTL_VIDEO_SET_COLOR_REGISTERS, pClut,
                           ClutSize, NULL, 0, &cColors)) {
        EngFreeMem(pClut);
        return FALSE;
    }
...
BOOL APIENTRY DrvSetPalette(IN DHPDEV dhpdev, IN PALOBJ *ppalo, IN FLONG fl, IN ULONG iStart,
    ...
    PaletteEntries = (PPALETTEENTRY)EngAllocMem(0, cColors * sizeof(ULONG), ALLOC_TAG);
